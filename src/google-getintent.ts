...
export async function doCommand(options: any) {
    ...
    function output(data) {
        const filePath = `${options.directory}/INTENT-${data.displayName}.json`;
...
export async function getIntent(intentGuid: string, options: any) {
    ...
    try {
        ...
        const formattedName = client.intentPath(options.projectId, intentGuid);

        // intentView is required to get Training sentences in data:
        const responses = await client.getIntent({ name: formattedName, intentView: 'INTENT_VIEW_FULL' });
        return responses[0];
    } catch (err) {
